...
public class MovementController : OVRPlayerController {
    ...
    protected override void UpdateController() {
        ...
        if ( Controller.isGrounded && FallSpeed <= 0 )
            FallSpeed = ((-Mathf.Abs(Physics.gravity.y) * (GravityModifier * 0.002f)));
        else
            FallSpeed += ((-Mathf.Abs( Physics.gravity.y ) * (GravityModifier * 0.002f)) * SimulationRate * Time.deltaTime);
